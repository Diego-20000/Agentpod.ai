...
### 4.1 Gaps cerrados (verificado contra la API real de Hetzner y prácticas de Fly.io/Railway/Render)

**Timeout del `POST change_type`** (mismo problema que el de crear servidor, §1.5): `change_type` no es instantáneo, Hetzner devuelve un `action_id` que queda `running/success/error`. Si el POST da timeout, **nunca reintentar de una** — reconciliar primero: si hay `action_id`, consultar la Action (`GET /actions/{action_id}`, el endpoint viejo `GET /servers/{id}/actions/{id}` está deprecated); si no hay `action_id` (timeout antes de recibir respuesta), consultar `GET /servers/{id}/actions` + `GET /servers/{id}` y verificar `server_type` actual antes de decidir si reintentar.

**`423 locked` no es un fallo del resize, es "esperá"**: significa que ya hay una Action corriendo sobre ese servidor (verificado: es el comportamiento documentado de la API, no un error real) — tratarlo como estado transitorio con reintento, no como `resize_status=failed`.

**Elegibilidad del target, dos momentos distintos**: `server_types.locations.available` en la API es solo un indicador del momento de la consulta, **no una garantía** de que el resize se pueda ejecutar después — puede fallar igual al momento real del `POST`. Antes de ofrecer un resize en el dashboard, pre-chequear: misma arquitectura (x86↔x86, arm↔arm — nunca CAX↔CX/CPX/CCX), mismo `storage_type`, y disco igual o mayor (nunca menor, ya cubierto arriba). CX/CPX↔CCX (shared↔dedicated) usa el mismo `change_type`, Hetzner lo llama "Rescale" pero no hay endpoint distinto.

**No hay cuota de Hetzner sobre "resizes por servidor"** — solo el límite general de API (3.600 requests/hora/proyecto) y el `423 locked` mientras una Action está en curso. El cooldown es una decisión nuestra, no un requisito de Hetzner: **un resize por pod a la vez** (no aceptar uno nuevo mientras el anterior no terminó) es obligatorio por coherencia de estado; un cooldown corto post-resize es opcional, para evitar que un usuario clickeando repetido genere conflictos de billing/lifecycle.

**Shutdown del agente antes de apagar: apuntar al workload, no solo al servicio MCP.** "Avisar a MCP/A2A para que el agente frene" es ambiguo — MCP/A2A no son el mecanismo de graceful shutdown de un proceso arbitrario (`npm run build`, `pytest`, `claude ...`). Mecanismo real, mismo patrón que usan Fly.io Machines, Railway y Render: `SIGTERM` al proceso/workload → grace period (Render usa 30s por defecto, configurable hasta 300s; mismo orden de magnitud en Fly/Railway) → si sigue vivo, `SIGKILL` → recién ahí Hetzner shutdown. La señal tiene que llegar a todo el árbol de procesos del workload, no solo al PID del servicio MCP — usar `systemd` con `KillMode=control-group` (mata todo el cgroup de la unidad, no solo el proceso principal), porque un proceso hijo detrás de un shell como PID 1 puede no recibir la señal si se manda solo al padre.

**Nota para logs/UI**: `server_type` (ej. `cx33`) es la fuente de verdad del plan activo — el CPU físico exacto (Intel/AMD) dentro de un mismo `server_type` compartido no lo es, Hetzner puede asignar cualquiera de los dos según disponibilidad, incluso cambiar en un rescale. No afirmar en logs/UI qué CPU físico específico tiene un pod.
